                          ...
                          const uint8_t *local_addr, uint16_t local_port, int passive)
{
    ...
    /* Setup active connection */
        ...
        _setup_timeout(&connection_timeout, \
            CONFIG_GNRC_TCP_CONNECTION_TIMEOUT_DURATION, \
    ...
    while (ret == 0 || tcb->pkt_retransmit != NULL) {
        ...
        switch (msg.type) {
            ...
            case MSG_TYPE_PROBE_TIMEOUT:
                ...
                if (probe_timeout_duration_us < \
                    CONFIG_GNRC_TCP_PROBE_LOWER_BOUND) {
                    probe_timeout_duration_us = \
                        CONFIG_GNRC_TCP_PROBE_LOWER_BOUND;
                }
                else if (probe_timeout_duration_us > \
                    CONFIG_GNRC_TCP_PROBE_UPPER_BOUND) {
                    probe_timeout_duration_us = \
                        CONFIG_GNRC_TCP_PROBE_UPPER_BOUND;
                }
                break;
