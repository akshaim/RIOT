    ...
    switch (newstate) {
        ...
        case GNRC_LWMAC_SLEEPING: {
            ...
            if (gnrc_lwmac_get_phase_backoff(netif)) {
                ...
                rtt_clear_alarm();
                alarm = random_uint32_range(RTT_US_TO_TICKS((3 * \
                    GNRC_LWMAC_WAKEUP_DURATION_US / 2)), \
...
static void _sleep_management(gnrc_netif_t *netif)
{
    ...
    if (!gnrc_lwmac_timeout_is_running(netif, GNRC_LWMAC_TIMEOUT_WAIT_DEST_WAKEUP)) {
        ...
        if (neighbour != NULL) {
            ...
            if (neighbour->phase > \
                RTT_TICKS_TO_US(CONFIG_GNRC_LWMAC_WAKEUP_INTERVAL_US)) {
            ...
            random_backoff = random_uint32_range(0, \
...
static void _rx_management_failed(gnrc_netif_t *netif)
{
    ...
    if (netif->mac.rx.rx_bad_exten_count >= \
        CONFIG_GNRC_LWMAC_MAX_RX_EXTENSION_NUM) {
    ...
    if (phase > (4 *\
        RTT_US_TO_TICKS(CONFIG_GNRC_LWMAC_WAKEUP_INTERVAL_US) / 5)) {
        gnrc_lwmac_set_quit_rx(netif, true);
    }
...
static void _rx_management_success(gnrc_netif_t *netif)
{
    ...
    /* If the relative phase is beyond 4/5 cycle time, go to sleep. */
    if (phase > \
        (4 * RTT_US_TO_TICKS(CONFIG_GNRC_LWMAC_WAKEUP_INTERVAL_US) / 5)) {
        gnrc_lwmac_set_quit_rx(netif, true);
    }
